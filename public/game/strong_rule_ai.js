...
(function(global){
  var factories = {
"../rules/added-kong": function(require, module, exports) {
"use strict";
...
function robKongWinner(state, owner, tile) {
    const players = state.players || [];
    for (let offset = 1; offset < players.length; offset += 1) {
        const playerId = (owner + offset) % players.length;
        const player = players[playerId];
        if (player && (0, hand_evaluator_1.canWin)(player.hand.concat(tile), { melds: player.melds || state.melds[playerId] || [], winTile: tile, winType: '抢杠' }).canWin)
            return playerId;
    }
    return undefined;
}
function resolveAddedKongDraw(input) {
    ...
    const melds = input.melds.map((meld, index) => index === pengIndex
        ? { type: 'mingGang', tiles: [input.kongTile, input.kongTile, input.kongTile, input.kongTile], fromPlayer: meld.fromPlayer }
        : meld);
    const handAfterDraw = handAfterKong.concat(input.drawTile);
    const win = (0, hand_evaluator_1.canWin)(handAfterDraw, { melds, winTile: input.drawTile, winType: '杠开' });
    if (!win.canWin) {
        return {
    ...
    const classification = (0, hand_evaluator_1.classifyHand)(handAfterDraw, melds, input.drawTile, '杠开');
    const settlementHand = handAfterDraw.concat(melds.flatMap((meld) => meld.tiles.slice(0, 3).filter((tile) => tile != null)));
    const settlement = (0, score_calculator_1.scoreSettlement)({ winner: input.owner, winType: '杠开', hand: settlementHand, scores: input.scores });
    return {
...
function hasMainThreatGenpai(defense) {
    if (!defense)
        return false;
    const maxThreat = Math.max(...defense.opponentModels.map((opponent) => opponent.tenpaiProbability));
    if (!Number.isFinite(maxThreat) || maxThreat < 0.75)
        return false;
    const safety = Array.from(defense.safetyPerTile.values())[0];
    if (!safety || !['safe', 'low', 'medium'].includes(safety.dangerLevel))
        return false;
    const mainThreats = new Set(defense.opponentModels
...
function windDefenseOverrideAdjustment(windComboBreak, defense) {
    if (!windComboBreak || !defense)
        return 0;
    if (defense.state.state === 'attack')
        return 0;
...
function severeBreakRank(candidate) {
    ...
    if (candidate.metadata.breaksKongCore)
        return 6;
...
function breaksConcealedKongCore(hand, tile, kongOpportunity) {
    return kongOpportunity && hand.filter((item) => item === tile).length >= 4;
}
function canUseGuardFallback(candidate) {
    if (candidate.metadata.dragonComboBreak)
        return false;
    if (candidate.metadata.breaksKongCore && !hasClearDefenseReason(candidate))
        return false;
    return true;
}
...
function finalDecisionGuard(candidates) {
    ...
    if (hasTenpaiCandidate && selected.metadata.isTenpaiAfter !== true)
        blockedReasonCodes.push('abandons-tenpai');
    if (selected.metadata.breaksKongCore)
        blockedReasonCodes.push('breaks-kong-core');
