...
console.log('=== calcShanten 测试 (14张手牌) ===');
...
for (let i = 0; i < hand.length; i++) {
  ...
  if (handKeys[i] === 'zhong' || handKeys[i] === 'tong1' || handKeys[i] === 'bai')
...
const zhongCands = bestCands.filter(c => c.tile === 'zhong');
const tong1Cands = bestCands.filter(c => c.tile === 'tong1');
console.log('\nzhong在候选?', zhongCands.length > 0 ? '是' : '否');
console.log('tong1在候选?', tong1Cands.length > 0 ? '是' : '否');

if (tong1Cands.length > 0 && zhongCands.length === 0) {
  console.log('\n✅ 修复成功：tong1在最佳候选，zhong不在');
} else if (tong1Cands.length > 0 && zhongCands.length > 0) {
  console.log('\n⚠️  zhone和tong1都在候选，需过滤阶段区分（预期正确）');
} else {
  console.log('\n❌ 修复不完整');
}
