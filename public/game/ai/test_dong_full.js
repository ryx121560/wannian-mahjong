// 全量测试：摸上tiao6后的手牌，所有候选的calcShanten
const fs = require('fs');
...
for (const r of results) {
  console.log(`  弃[${r.idx}] ${r.tile}: shanten=${r.shanten}`);
}

const bestS = results[0].shanten;
const bestCands = results.filter(r => r.shanten === bestS);
console.log(`\n最佳向听数: ${bestS}`);
