import { canDeclareSpecialKongAction, getLegalActions, resolveKongDraw, resolveRobKongWinner, resolveSpecialKongAction, scoreKongSettlement, scoreSpecialKongSettlement } from '../rules';
...
function player(state: GameState, playerId: number): { hand: Tile[]; melds: Meld[] } {
  const value = state.players?.[playerId];
  if (!value) throw new Error(`missing stage8 v2 player: ${playerId}`);
  return { hand: value.hand.slice(), melds: (value.melds || state.melds[playerId] || []).slice() };
}

function uniqueTiles(hand: Tile[]): Tile[] {
  return Array.from(new Set(hand)).sort();
}

function count(hand: Tile[], tile: Tile): number {
...
function declarationWindow(state: GameState): 'discard-response' | 'self-draw-discard' {
  return state.phase === 'responding' ? 'discard-response' : 'self-draw-discard';
}

function rawRuleActions(state: GameState, playerId: number, legal: LegalAction[], candidateKongResources: CandidateConcealedKongResource[], addedKongChainWindows: AddedKongChainWindowInput[]): CanonicalStage8V2Action[] {
  const current = player(state, playerId);
  const window = declarationWindow(state);
  ...
  if (legal.includes('discard')) for (const tile of uniqueTiles(current.hand)) actions.push(canonicalizeStage8V2Action({ actionType: 'discard', actor: playerId, declarationWindow: 'self-draw-discard', tile, robKongWindow: false }));
  ...
  if (legal.includes('concealedKong')) for (const tile of uniqueTiles(current.hand).filter((tile) => count(current.hand, tile) === 4)) {
    const handAfterKong = removeTiles(current.hand, tile, 4);
  ...
  for (const selectedResource of activeResources) for (const conditionalResource of activeResources) {
    ...
    const handAfterKong = removeTiles(current.hand, selectedResource.tile, 1);
  ...
  for (const windowState of addedKongChainWindows.filter((windowState) => windowState.owner === playerId)) {
    ...
    const handAfterChainKong = removeTiles(current.hand, chainTile, 1);
...
export function deriveStage8V2RuleActions(input: Stage8V2VisibleActionInput): CanonicalStage8V2Action[] {
  assertStage8V2Protocol(input as unknown as Record<string, unknown>);
  return rawRuleActions(input.state, input.playerId, getLegalActions(input.state, input.playerId), input.candidateKongResources || [], input.addedKongChainWindows || []);
}

function ruleExecutionOwner(input: Stage8V2KongExecutionInput): number {
...
function ruleExpectedActionType(input: Stage8V2KongExecutionInput): Stage8V2ActionType {
  if (input.claim.family === 'decline') return 'declineKong';
  const kind = input.claim.action.kind;
  if (kind === 'addedKongChain') return 'chainKong';
  return kind;
}

function ruleRobTile(input: Stage8V2KongExecutionInput): Tile | null {
...
function rulePlayerSnapshot(state: GameState, owner: number): { hand: Tile[]; melds: Meld[] } {
  const value = state.players?.[owner];
  if (!value) throw new Error('stage8-v2-rule-execution-player-missing');
  return {
...
function rulePublicResult(
  ...
  const owner = ruleExecutionOwner(input);
  return {
...
export function executeStage8V2RuleKongAction(input: Stage8V2KongExecutionInput): Stage8V2KongExecutionResult {
  assertStage8V2Protocol(input as unknown as Record<string, unknown>);
  const owner = ruleExecutionOwner(input);
  const expectedActionType = ruleExpectedActionType(input);
  if (input.selectedAction.actionSpaceVersion !== STAGE8_ACTION_SPACE_V2_VERSION
    || input.selectedAction.actionType !== expectedActionType
    || input.selectedAction.context.actor !== owner) {
    throw new Error('stage8-v2-rule-selected-action-mismatch');
  }
  const current = rulePlayerSnapshot(input.state, owner);
  if (input.claim.family === 'decline') {
    return rulePublicResult(input, {
  ...
  const robTile = ruleRobTile(input);
  ...
  if (!drawTile) throw new Error('stage8-v2-rule-supplement-unavailable');
  if (input.claim.family === 'kongResource') {
