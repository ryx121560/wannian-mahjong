import type { GameState, LegalAction, Meld, Tile } from '../rules';
...
import type { Stage8V2KongExecutionResult, Stage8V2KongPageExecutionInput, Stage8V2PageSemanticInput } from './v2-visible-state';

function pagePlayer(state: GameState, playerId: number): { hand: Tile[]; melds: Meld[] } {
  const value = state.players?.[playerId];
  if (!value) throw new Error(`missing stage8 v2 page player: ${playerId}`);
  return { hand: value.hand.slice(), melds: (value.melds || state.melds[playerId] || []).slice() };
}

function pageUniqueTiles(hand: Tile[]): Tile[] {
  return Array.from(new Set(hand)).sort();
}

function pageCount(hand: Tile[], tile: Tile): number {
  return hand.filter((item) => item === tile).length;
}

function pageRemoveTiles(hand: Tile[], tile: Tile, amount: number): Tile[] | null {
...
function pageWindow(state: GameState): 'discard-response' | 'self-draw-discard' {
  return state.phase === 'responding' ? 'discard-response' : 'self-draw-discard';
}

function pageActions(state: GameState, playerId: number, legal: LegalAction[], canDeclareSpecialKongAction: (action: unknown) => boolean, candidateKongResources: CandidateConcealedKongResource[], addedKongChainWindows: AddedKongChainWindowInput[]): CanonicalStage8V2Action[] {
  const current = pagePlayer(state, playerId);
  const window = pageWindow(state);
  ...
  if (legal.includes('discard')) for (const tile of pageUniqueTiles(current.hand)) actions.push(canonicalizeStage8V2Action({ actionType: 'discard', actor: playerId, declarationWindow: 'self-draw-discard', tile, robKongWindow: false }));
  ...
  if (legal.includes('concealedKong')) for (const tile of pageUniqueTiles(current.hand).filter((tile) => pageCount(current.hand, tile) === 4)) {
    const handAfterKong = pageRemoveTiles(current.hand, tile, 4);
  ...
  for (const selectedResource of activeResources) for (const conditionalResource of activeResources) {
    ...
    const handAfterKong = pageRemoveTiles(current.hand, selectedResource.tile, 1);
  ...
  for (const windowState of addedKongChainWindows.filter((windowState) => windowState.owner === playerId)) {
    ...
    const handAfterChainKong = pageRemoveTiles(current.hand, chainTile, 1);
...
export function deriveStage8V2PageSemanticActions(input: Stage8V2PageSemanticInput): CanonicalStage8V2Action[] {
  assertStage8V2Protocol(input as unknown as Record<string, unknown>);
  return pageActions(input.state, input.playerId, input.browserRuleEngine.getLegalActions(input.state, input.playerId), input.browserRuleEngine.canDeclareSpecialKongAction, input.candidateKongResources || [], input.addedKongChainWindows || []);
}

function pageExecutionOwner(input: Stage8V2KongPageExecutionInput): number {
...
function pageExpectedActionType(input: Stage8V2KongPageExecutionInput): Stage8V2ActionType {
...
function pageExecutionRobTile(input: Stage8V2KongPageExecutionInput): Tile | null {
...
function pageExecutionPlayer(state: GameState, owner: number): { hand: Tile[]; melds: Meld[] } {
  const value = state.players?.[owner];
  if (!value) throw new Error('stage8-v2-page-execution-player-missing');
  return {
...
function pageExecutionResult(
  input: Stage8V2KongPageExecutionInput,
  ...
  const owner = pageExecutionOwner(input);
  return {
...
export function executeStage8V2PageKongAction(input: Stage8V2KongPageExecutionInput): Stage8V2KongExecutionResult {
  assertStage8V2Protocol(input as unknown as Record<string, unknown>);
  const owner = pageExecutionOwner(input);
  if (input.selectedAction.actionSpaceVersion !== STAGE8_ACTION_SPACE_V2_VERSION
    || input.selectedAction.actionType !== pageExpectedActionType(input)
    || input.selectedAction.context.actor !== owner) {
    throw new Error('stage8-v2-page-selected-action-mismatch');
  }
  const current = pageExecutionPlayer(input.state, owner);
  if (input.claim.family === 'decline') {
    return pageExecutionResult(input, {
  ...
  const robTile = pageExecutionRobTile(input);
  if (input.selectedAction.context.robKongWindow && robTile) {
    const robKongWinner = input.browserRuleEngine.resolveRobKongWinner(input.state, owner, robTile);
  ...
  if (!drawTile) throw new Error('stage8-v2-page-supplement-unavailable');
  if (input.claim.family === 'kongResource') {
    const action: any = { ...input.claim.action, drawTile };
    const resolution = input.browserRuleEngine.resolveKongDraw(action);
    const settlement = resolution.mustDiscard
      ? null
      : input.browserRuleEngine.scoreKongSettlement({
        action,
        winner: owner,
    ...
    return pageExecutionResult(input, {
      ...
      nextMelds: action.melds.map((meld: Meld) => ({ ...meld, tiles: meld.tiles.slice() as Meld['tiles'] })),
  ...
  const resolution = input.browserRuleEngine.resolveSpecialKongAction(action);
  const settlement = resolution.mustDiscard
    ? null
    : input.browserRuleEngine.scoreSpecialKongSettlement({ action, winner: owner, scores: input.state.scores });
  const classification = resolution.evaluation.classification;
  ...
  return pageExecutionResult(input, {
    ...
    wallConsumed: 1,
    nextHand,
    nextMelds: action.input.melds.map((meld: Meld) => ({ ...meld, tiles: meld.tiles.slice() as Meld['tiles'] })),
    resourceAfterKong: resolution.resourceAfterKong,
    settlement,
    handTypes: classification?.handTypes.slice() || [],
    decompositionSignature: classification?.decompositionSignature || resolution.evaluation.decomposition?.signature || null,
