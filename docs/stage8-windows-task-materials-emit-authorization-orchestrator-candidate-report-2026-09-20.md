# Stage 8 Windows Task materials-emit 阶段授权原子签发发布报告

日期：2026-09-20
状态：正式发布范围已冻结；最终提交与推送结果仅记录于外部发布证据；未部署、未执行正式签发

## 结论

本次发布在已发布 check-only 能力上增加一个最小原子入口，仅支持：
...
- 分支：`codex/stage8-windows-task-materials-emit-authorization-publish-20260920`
- 工作树：独立干净发布工作树
- 新依赖：无
- 数据库变更：无
- 精确项目文件：4 个
...
本报告不回填本次发布提交的 commit-bound 身份；最终提交产生后，其源码身份仅作为外部发布证据记录。
...
直接运行 `npx tsc --noEmit --incremental false` 时，发布工作树和根 `node_modules` 均缺少项目既有依赖 `onnxruntime-node`，因此准确失败为 `offline-onnx-inference-adapter.ts` module-not-found 及 3 个派生 implicit-any；该次结果不记为通过，也没有安装或复制依赖。

随后复用已验工作树 `codex-stage8-bc-sample-probe-20260901/node_modules/onnxruntime-node@1.27.0`：在项目外创建临时 tsconfig，继承发布工作树的 `tsconfig.json`，保留 `@/* -> 发布工作树/src/*`，并把 `onnxruntime-node` 映射到该现有共享包；执行 `npx tsc --noEmit --incremental false --pretty false -p <临时配置>` 通过。临时配置随后删除，发布工作树项目文件、`node_modules` 和依赖清单均未改变。
...
未访问用户 Storage、对局或导出；未运行 diagnostic、Pilot、训练、部署或 18768。本次发布只交付默认拒绝的协议代码，不执行正式阶段授权签发。

## 后续门禁

协议代码发布完成后只报验；代码发布不等于正式签发授权。正式 `--emit-and-verify` 必须在 signer identity 干净且获得新的精确产品授权后才能执行；签发完成也不自动授权材料生成、register、run、verify 或 delete。
