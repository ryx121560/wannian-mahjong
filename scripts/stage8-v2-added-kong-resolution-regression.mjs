...
try {
  ...
  assert.equal(immediate.settlement.delta.reduce((sum, value) => sum + value, 0), 0, 'immediate settlement score deltas must balance');
  assert.deepEqual(immediate.settlement.after.map((value, index) => value - immediate.settlement.before[index]), immediate.settlement.delta, 'immediate settlement score deltas must match before and after scores');
