...
try {
  ...
  const first = engine.executeStage8OfflineSelfplayDecision({ cursor, plan, game, smokeControl, artifactRoot, rawDistributionProvider: uniformProvider, providerIdentitySha256: fixed });
  ...
  const replay = engine.executeStage8OfflineSelfplayDecision({ cursor: engine.createStage8OfflineSelfplayCursor(structuredClone(state)), plan, game, smokeControl, artifactRoot, rawDistributionProvider: uniformProvider, providerIdentitySha256: fixed });
  ...
  const badProvider = (request) => ({ [identityTools.stage8CanonicalActionKey(request.legalActions[0])]: 1 });
  const fused = engine.executeStage8OfflineSelfplayDecision({ cursor, plan, game, smokeControl, artifactRoot, rawDistributionProvider: badProvider, providerIdentitySha256: fixed });
  ...
  const denied = engine.executeStage8OfflineSelfplayDecision({ cursor, plan, game, smokeControl: deniedControl, artifactRoot, rawDistributionProvider: uniformProvider, providerIdentitySha256: fixed }); assert.equal(denied.ok, false); assert.equal(denied.reason, 'smoke-explicit-authorization-required');
  const chainAction = { actionSpaceVersion: 'stage8-action-space-v2', actionType: 'chainKong', actionId: 900, tile: 'wan1', context: { actor: 0, declarationWindow: 'chain-kong', ownTileCount: 1, robKongWindow: true, resourceSignature: '0:wan2>wan1' } }; const discardAction = { actionSpaceVersion: 'stage8-action-space-v2', actionType: 'discard', actionId: 200, tile: 'wan1', context: { actor: 0, declarationWindow: 'self-draw-discard', ownTileCount: 1, robKongWindow: false } }; const chainKeys = [chainAction, discardAction].map(identityTools.stage8CanonicalActionKey).sort(); const raw = Object.fromEntries(chainKeys.map((key) => [key, 0.5])); const chainSelection = behavior.selectStage8OfflineBehaviorAction({ visibleState: { actor: 0 }, legalActions: [chainAction,discardAction], rawDistributionProvider: () => raw, providerIdentitySha256: fixed, decisionIdentity: fixed, scenario: 'chainKong', candidateSeat: 0, actor: 0 }); assert.equal(chainSelection.ok, true); assert.deepEqual(chainSelection.value.behaviorActionDistribution, raw, 'chain is report-only and does not receive targeted mixing'); assert.equal(chainSelection.value.exploration, false);
